...
3. **Trace PFL outputs to identified descending cells and onward through VNC.** Brain-side trace done 2026-09-10; see [descending-trace.md](descending-trace.md). VNC not in this export. Use our ranked one-hop table to choose targets, then add intermediates and normalize by each target's total input. Large raw counts are not automatically selective or functionally powerful.
...
5. **Compare matched circuits across specimens.** Compass circuit compared with hemibrain v1.2 on 2026-09-10 (conserved; see [compass-recurrence.md](compass-recurrence.md)). FC2/PFL/EL comparison still open. MaleCNS versus hemibrain/FlyWire can test recurrence, subtype counts, and asymmetry. Specimen and sex are confounded in a one-male/one-female comparison; a difference does not by itself establish sexual dimorphism.
6. **Build a constrained dynamical model only after the mapping audit.** Two minimal models run 2026-09-11 (EB EPG→PEN loop; PFR integrator), see [uncharacterised-neurons.md](uncharacterised-neurons.md) section C. Full fitted model still open.
7. **Assign functions to uncharacterised cells.** Started 2026-09-11: [uncharacterised-neurons.md](uncharacterised-neurons.md) (targeted candidates plus unbiased screen) and [behavioral-task-map.md](behavioral-task-map.md) (return-to-food decomposition). Fit against published tuning, perturbation, drift, and steering data. Hold out perturbations for validation. A plausible bump generated by arbitrary weights is not a mechanistic validation of this connectome.
