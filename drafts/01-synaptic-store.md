...
> **Audit note (2026-09-13).** Re-analysis in [docs/audit-2026-09-13.md](../docs/audit-2026-09-13.md) withdraws the claim that activity-based storage is ruled out (the same recurrence metric gives the known compass ring a lower score than several fan-shaped-body populations, and the original table had a column-binning bug), and corrects the write and reset coverage: FB4M covers every cell of hΔA and hΔI only; hΔH's uniform dopamine is FB5H; hΔG has no dopamine coverage; OA-VPM3 is uniform on hΔH and hΔG, partial on hΔA, absent on hΔI; the largest modulatory input to hΔA and hΔI is the serotonergic FB4Y. The site simulations quoted below were run in label space; rerun in the axon frame the direct routes walk away and the inverting routes return, as the text says. hΔA and hΔH remain the strongest candidates.
