...
> **Audit note (2026-09-13).** PS196 as GLNO's largest input is in print (Hulse et al. 2023, bioRxiv 2023.09.25.559373, which also proposes GLNO inherits its velocity estimate from it); the fan-out to LPsP, ExR2, ExR4 and FB3A is the part not in that paper. PS196_b receives no descending input among its main sources but does receive the ascending neuron AN07B037 (7 %), which argues for a body-derived turning signal rather than a motor-command copy. FB3A's ascending input (AN06B009, 3 %) is smaller than its visual-motion input (LAL143 from LLPC1, 6 %) and its antennal input (PS326 from Johnston's-organ types, 4 %), so it pools several self-motion cues. Details: [docs/audit-2026-09-13.md](../docs/audit-2026-09-13.md).
