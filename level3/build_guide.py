...
<h4>What "write strength" means, and when writing happens</h4>
<p>Three things are easy to tangle here, so here they are one at a time.</p>
<p><b>When does writing happen?</b> On every tick, on every cell, without exception. There is no switch in the machine that says "write now" or "do not write". What varies is how much is written: the write gate scales each tick's addition between nothing and the full amount. So the gate answers "how much this tick", and nothing in the machine answers "when". Writing is always on.</p>
<p><b>What is ws, then?</b> An exchange rate between two kinds of number. The pairing, two activities multiplied, is in units of activity and lies between −1 and +1. The cell is in units of connection strength and has a lid at ±1. ws says how much connection strength one full-strength tick of pairing is worth. At ws = 0.074, a tick in which two neurons are fully active together, with the gate fully open, moves their wire by 0.074. About fourteen such ticks fill a cell to the lid. So ws sets the tempo of the count: how many ticks of walking fit on one wire before it is full.</p>
<p><b>What did training ws mean?</b> Training moved ws by the same downhill steps as every other parameter, and it settled where homing was best. It was balancing two failures. If ws is too large, cells reach the lid a few seconds into a wander and every step after that is not counted. If ws is too small, the count stays faint, the reading side has to amplify it a great deal through A, and the small always-on fade and the compass noise swamp it. The value 0.074 is where run 19 landed: most of a thirty-second wander fits under the lid while the count is still loud enough to read.</p>
<p>One correction to a phrase that is easy to reach for: the write gate is not "tied to the fast weights". It is computed from the 64 activities through W_out, which is a slow, trained table. It is recomputed every tick because the activities change every tick, not because it lives in F.</p>
<div class="sketch">
 <div class="eyebrow">Drawing: one cell over a thirty-second wander, for three values of ws</div>
 <canvas id="wsThree" width="1700" height="440"></canvas>
 <div class="how"><b>How to read it.</b> Time runs left to right for thirty seconds (300 ticks) of walking in one direction, with the write gate held at 0.4 and a steady pairing of 0.5. Three lines, one per ws value. At ws = 0.3 the cell hits the lid in about two seconds and stops counting. At ws = 0.02 the cell is still far from the lid at thirty seconds but has only reached 0.6 of the lid's height, which makes it a faint signal to read. At ws = 0.074, run 19's value, the cell fills at a pace that roughly matches the length of the wanders it was trained on. Training found this middle by trial, not by design.</div>
</div>
<div class="panel real">
 <div class="eyebrow">Real: ws during training, runs 3 and 19</div>
 <canvas id="wsTrain" width="1700" height="400"></canvas>
 <div class="how"><b>How to read it.</b> Across: training iterations. Up: the value of ws at that point in training. Run 3 started from the initial value of 0.10 and training moved it down to 0.066 over about three thousand iterations. Run 19 started from run 14's weights (a warm start) at 0.082 and drifted down to 0.074 over its eight hundred iterations. ws is a parameter like any other: it drifts during training and is frozen at the end, and the value it lands on is what every exam trip uses.</div>
</div>
...
(function(){const [x,W,H]=ctx('wsThree');x.clearRect(0,0,W,H);const p={l:44,r:20,t:26,b:40};const cw=W-p.l-p.r,ch=H-p.t-p.b;const N=300;const X=k=>p.l+k/N*cw,Y=v=>H-p.b-v/1.1*ch;frame(x,W,H,p);[0,0.5,1].forEach(v=>txt(x,v,p.l-6,Y(v)+4,css('--mute'),'right'));[0,10,20,30].forEach(s=>txt(x,s+' s',X(s*10),H-p.b+16,css('--mute'),'center'));x.setLineDash([6,5]);x.strokeStyle=css('--coral');x.lineWidth=2;x.beginPath();x.moveTo(p.l,Y(1));x.lineTo(W-p.r,Y(1));x.stroke();x.setLineDash([]);txt(x,'lid',W-p.r-4,Y(1)-6,css('--coral'),'right');
 [[0.3,css('--rand')],[0.074,css('--teal')],[0.02,css('--blue')]].forEach(([ws,col])=>{let v=0;x.strokeStyle=col;x.lineWidth=3;x.beginPath();for(let k=0;k<=N;k++){k?x.lineTo(X(k),Y(v)):x.moveTo(X(k),Y(v));v=Math.min(1,v+ws*0.4*0.5);}x.stroke();const hit=Math.ceil(1/(ws*0.4*0.5));txt(x,`ws = ${ws}${hit<=N?' · full after '+(hit/10).toFixed(1)+' s':' · at '+ (Math.min(1,N*ws*0.2)).toFixed(2)+' after 30 s'}`,X(Math.min(N,hit)+4),Y(Math.min(1,hit*ws*0.2))+(ws===0.3?18:-8),col,'left','13px "Bricolage Grotesque"');});})();
(function(){const [x,W,H]=ctx('wsTrain');x.clearRect(0,0,W,H);const p={l:54,r:20,t:26,b:40};const cw=W-p.l-p.r,ch=H-p.t-p.b;const T=D.motif.ws_train;const itmax=Math.max(...T.run3.map(a=>a[0]));const X=it=>p.l+it/itmax*cw,Y=v=>H-p.b-v/0.12*ch;frame(x,W,H,p);[0,0.05,0.1].forEach(v=>txt(x,v,p.l-6,Y(v)+4,css('--mute'),'right'));[0,1000,2000,3000,4000].forEach(it=>{if(it<=itmax)txt(x,it,X(it),H-p.b+16,css('--mute'),'center');});txt(x,'training iterations →',W-p.r,H-6,css('--mute'),'right');txt(x,'ws',p.l+4,p.t-8,css('--mute'));
 const line=(arr,col,label)=>{x.strokeStyle=col;x.lineWidth=3;x.beginPath();arr.forEach((a,k)=>k?x.lineTo(X(a[0]),Y(a[1])):x.moveTo(X(a[0]),Y(a[1])));x.stroke();const e=arr[arr.length-1];txt(x,`${label}: ends at ${e[1].toFixed(3)}`,X(e[0])+6,Y(e[1])+4,col,'left','13px "Bricolage Grotesque"');};
 line(T.run3,css('--ink'),'run 3 (cold start from 0.10)');line(T.run19,css('--teal'),'run 19 (warm start)');})();
