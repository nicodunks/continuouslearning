...
<div class="sketch">
 ...
 <div class="row"><button class="btn" id="fb0">start: F before</button><button class="btn" id="fb1">move 1: shrink by (1 − erase)</button><button class="btn" id="fb2">move 2: add ws × write × deposit</button><button class="btn" id="fb3">move 3: clamp to ±1</button><button class="btn" id="fbNext">make this the new "before"</button><button class="btn" id="fbTen">ten full ticks with the pen down (write 1, erase 0)</button></div>
 <canvas id="fboard" width="1700" height="520"></canvas>
 <div class="how"><b>Why (1 − erase) and not × erase.</b> The gate is named for what it removes. Erase 0.28 means 28% of each cell goes, 72% stays, and "keep 72%" is × (1 − 0.28). Multiplying by the gate directly would make erase = 1 keep everything, the word backwards. Either convention works mathematically (training would learn the opposite sign); this one makes "erase fires at food" read like the fly's reset. The LSTM's "forget gate" uses the other convention, which is why that name confuses people.</div>
<div class="how"><b>Why move 3 often changes nothing.</b> The clamp only touches cells already past +1 or −1. After one deposit on this board, none are, so nothing happens. Press "ten full ticks with the pen down" and watch cells climb until the clamp bites: clipped cells get an orange border and the count is written below. That is the lid from the Counter page, cell by cell.</div>
...
<div class="sketch">
 ...
 <svg viewBox="0 0 900 170">
  <rect x="30" y="40" width="250" height="90" rx="10" fill="var(--panel)" stroke="var(--line)"/><text x="155" y="70" text-anchor="middle" font-family="Bricolage Grotesque" font-size="15" font-weight="700" fill="var(--ink)">ws = 0.074</text><text x="155" y="92" text-anchor="middle" font-family="Literata" font-size="13" fill="var(--mute)">one trained number</text><text x="155" y="112" text-anchor="middle" font-family="Literata" font-size="13" fill="var(--mute)">never changes during a trip</text>
  <text x="310" y="90" font-family="Bricolage Grotesque" font-size="26" fill="var(--ink)">×</text>
  <rect x="350" y="40" width="250" height="90" rx="10" fill="var(--panel)" stroke="var(--amber)" stroke-width="2"/><text x="475" y="70" text-anchor="middle" font-family="Bricolage Grotesque" font-size="15" font-weight="700" fill="var(--ink)">write(t) = 0 to 1</text><text x="475" y="92" text-anchor="middle" font-family="Literata" font-size="13" fill="var(--mute)">one number per tick, shared</text><text x="475" y="112" text-anchor="middle" font-family="Literata" font-size="13" fill="var(--mute)">by all 4,096 connections</text>
...
<div class="sketch">
 <div class="eyebrow">Drawn · one tick by hand, on a three-neuron version of the same machine</div>
...
<div class="panel">
 ...
 <tr><td class="n">−5</td><td class="n">0.0067</td><td class="n">0.0066</td><td>where the erase gate started: nearly shut, nearly deaf</td></tr>
 <tr><td class="n">−2.5</td><td class="n">0.076</td><td class="n">0.070</td><td>after the first shift: ten times louder</td></tr>
 <tr><td class="n">0</td><td class="n">0.5</td><td class="n">0.25</td><td>the loudest point on the curve</td></tr>
 <tr><td class="n">+1.86</td><td class="n">0.87</td><td class="n">0.11</td><td>run 19's trained resting position</td></tr>
 <tr><td class="n">+5</td><td class="n">0.9933</td><td class="n">0.0066</td><td>deaf again, at the top</td></tr>
 </tbody></table>
 <p style="font-size:15px"><b>Why the slope multiplies the nudge.</b> Training wants to change the gate's output. It can only change the raw z (through the bias and W_out). A small change in z changes the output by slope × that change: that is what slope means. So every gradient that reaches the gate is multiplied by the slope on its way to the bias. At −5 the multiplier is 0.0066. A push that would have moved the gate by 0.1 at z = 0 moves it by 0.0026 at z = −5. The gate is not stubborn; it is being whispered to.</p>
</div>
...
<div class="panel real">
 ...
 <div class="how"><b>How to read it.</b> One small chart per neuron, headings east, north, west, south, east across the bottom, activity up. Teal curves are the 32 that pass the 0.25 test; grey are the rest. The number is the tuning strength. Notice that the tuned cells are not all copies: their peaks fall in different places. The histogram below counts them by preferred direction. A perfect compass would spread evenly around the circle; ours clusters (six cells prefer north-north-east, six prefer west-south-west, and two of the sixteen bins are empty), which is one of the ways it is still cruder than the fly.</div>
