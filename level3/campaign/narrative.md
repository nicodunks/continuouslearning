...
## The ratio is a fixed point, but the peaks grow (run 10)
A second bias shift did not lift the mean food-versus-away ratio past about 7.5; that number seems to be where this signal settles. What the second shift did change is the shape of the erase trace: instead of a slightly raised plateau at food, there are now peaks reaching 0.20 on the test trip and 0.2 to 0.3 during training, against 0.06 before. The gate is learning a spike, one tick at a time, and averaging over a four-second stand hides it. It also produced the best earned 30-second score of the night, 2.10 on the standard exam, with frozen checks under 1.5. So the two goals are not in conflict: the runs that moved the reset forward also moved the score.
